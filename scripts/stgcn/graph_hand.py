...
class Graph:
    """
    Full ST-GCN graph that returns A with shape (K, V, V)
    using the "spatial" partition strategy.
    """
    ...
    def get_adjacency(self, strategy):
        ...
        if strategy == "spatial":
            # Partition into: root (same distance to center), close (closer to center), further (further from center)
            A = []
            for hop in valid_hop:
